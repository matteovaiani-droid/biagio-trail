...
if (!url || !key) {
  throw new Error("Variabili Supabase non trovate nel file .env");
}
...
async function main() {
  ...
  console.log(`Da processare: ${trails?.length ?? 0}`);

  let updated = 0;

  for (const trail of trails ?? []) {
    try {
      console.log(`➡️ ${trail.name} (${trail.osm_id})`);
...
`;
      ...
      if (!firstPoint) {
        console.log("⚠️ Punto iniziale mancante");
        continue;
      }
      ...
      console.error(`❌ Errore ${trail.osm_id}`);
  ...
  console.log(`🎉 Completato. Aggiornati ${updated} sentieri`);
}
